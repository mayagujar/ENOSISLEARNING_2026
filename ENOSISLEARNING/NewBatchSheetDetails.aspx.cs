        ...
        public static string AddBatchSheet(string Date, string ChapterId, string Topic, string Comments, string FileName)
        {
            try
            {
                string batchId = HttpContext.Current.Session["BatchId"]?.ToString();
                 string facultyName = HttpContext.Current.Session["FacultyName"]?.ToString();
                ...
                (Date, ChapterNumber, TopicCovered,BatchId, uploads,Comments,Status,CREATEDBY)
                VALUES
                (@Date, @Chapter, @Topic, @BatchId, @Uploads, @Comments, 'Present',@CREATEDBY)", con);
                    ...
                    cmd.Parameters.AddWithValue("@CREATEDBY", facultyName);
